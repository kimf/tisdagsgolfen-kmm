...
import se.fransman.tg.shared.viewmodel.screens.navigationSettings
...
class StateManager(repo: Repository) {
  ...
  fun setupNewLevel1Screen(screenIdentifier: ScreenIdentifier) {
    ...
    if (navigationSettings.alwaysQuitOnHomeScreen) {
      if (screenIdentifier.URI == navigationSettings.dashboardScreen.screenIdentifier.URI) {
        level1Backstack.clear() // remove all elements
      } else if (level1Backstack.size == 0) {
        addLevel1ScreenToBackstack(navigationSettings.dashboardScreen.screenIdentifier)
      }
    }
